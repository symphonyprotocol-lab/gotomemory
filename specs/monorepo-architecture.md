...
> 本规格把产品规格（[memory-sharing-system.md](memory-sharing-system.md)）落成**可直接照着写代码的工程骨架**：目录怎么分、包之间谁能依赖谁、契约从哪来、本地优先怎么在代码里体现、构建/测试/发布怎么跑。
>
> 原则一句话：**浏览器扩展是产品本体且本地优先，所以核心逻辑必须是平台无关的纯 TS 库，扩展/网页/服务器只是它的三个外壳；服务器只承担用户显式开启后的同步。**
...
3. **契约单一来源**：本地操作与云端同步共用同一套类型，由 `packages/contracts` 从 OpenAPI + JSON Schema 生成（对应 memory 规格 §8「本地与云端说同一种话」）。
...
| Web 前端 | **Vite + React**（dev 端口 5173） | `@gotomemory/web` |
...
│  │                    # 各自加载对应 site-adapter，挂"带入面板/保存气泡/导出"入口
│  ├─ popup/            # 快捷面板
...
- **同步上线后**：不改 `core`，只在 background 增加一个把本地变更推到同步服务的同步器（消费 `contracts` 客户端），`rev`/`deleted_at` 字段已为此预留（memory 规格 §7）。
...
| 服务端不记录私密正文 | 同步服务日志中间件（后续） | memory §10 |
...
- **`py/sdk` + MCP/CLI**：开发者/Agent 接入（`search_memory`/`save_memory`/`build_context` 工具）。可加 `apps/mcp-server`（streamable-http + OAuth）复用 `contracts`，作为高级补充，不替代扩展主入口。
- **多租户/策略引擎/审计**：在 `contracts` 与未来同步服务上做向上兼容扩展（`tenant_id`、policy、审计哈希链），不污染消费主线。
